...
import Icon from "@main/GetIcon";
import UpdateValuesBale from "@main/UpdateBale";
...
export default function Alert({
	...
	}, [internalState.isProcessing, ws, closeAlert, data?.idUnique]);
	...
	const alertContent = useMemo(() => {
		...
		const wsNotConnected = String(currentMessage)
			.toLowerCase()
			.includes("websocket");
		...
		switch (currentAlertType) {
			...
			case "info": {
				return (
					<div className="alert-box on-border note">
						<h1
							className="flex items-center title-alert text-left"
							style={{ color: "black" }}
						>
							<Icon type="working" />
							{title || "Nota scritta dall'utente"}
				...
				return (
					<div className="alert-box on-border update !bg-slate-50">
						...
						>
							<IoClose
								size={30}
								className="text-white fixed bg-red-400 right-3 top-3 rounded-full"
							/>
						</button>
