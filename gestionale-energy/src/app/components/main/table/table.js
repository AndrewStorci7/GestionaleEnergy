...
import DownloadReport from "@/app/components/main/admin/buttons/BtnReport";
import BtnReportFiltered from "@/app/components/main/admin/buttons/BtnReportFiltered";
import Switch from "@/app/components/main/admin/Switch";
...
import { useWebSocket } from "@main/ws/WebSockerProvider";
...
export default function Table({ type, implant, idUser }) {
	...
	const _CMNSTYLE_EMPTY =
		"text-2xl w-screen min-h-screen flex justify-center items-center";
	...
	switch (type) {
		...
		case "presser": {
			return (
				<>
					...
					{!addWasClicked ? (
						<div
							className={`${isEmpty || addWasClicked ? "visible" : "invisible"} ${_CMNSTYLE_DIV_EMPTY}`}
						>
							<h1 className={`${_CMNSTYLE_EMPTY}`}>{msgEmpty}</h1>
						</div>
					) : null}
				</>
			);
		...
		case "wheelman": {
			return (
				<>
					<BtnWheelman implant={implant} idUser={idUser} baleObj={objIdBale} />
		...
		case "both": {
			return (
				<>
						...
						<div className="flex gap-4">
							<div className="w-fit flex justify-between items-center space-x-4">
								<div className="flex items-center">
									<p className={!handleToggleSwitch ? "font-semibold" : ""}>
										Amministratore
									</p>
								</div>
								<div className="flex items-center">
							...
							<div className="relative w-fit ml-5">
								...
								>
									<div
										className={`flex items-center p-1 text-white ${!handleToggleSwitch && "disabled:bg-blue-200"}`}
									>
										<Image
					...
					{!handleToggleSwitch ? (
						<div
							className="grid grid-cols-2 gap-2 relative rounded-lg
                                        sm:h-[40vh]
                                        md:h-[55vh]
                                        lg:h-[80vh]"
						>
