...
export default function BtnWheelman({ baleObj }) {
			...
			if (type === "print") {
				...
				showAlert({
					...
					message:
						type === "update"
							? null
							: `Sei sicuro di voler stampare la balla ${baleObj.idUnique} ?`,
					type: type === "update" ? "update-w" : "confirm",
					onConfirm: () =>
						type === "update"
							? null
							: handleStampa(baleObj, showAlert, hideAlert, true),
					data: baleObj,
				});
			}
			refreshPage(ws);
