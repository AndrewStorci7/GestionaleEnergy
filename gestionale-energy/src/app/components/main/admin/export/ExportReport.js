 ...
 */
export const handleDownload = async (data, reportInfo, dateForReport) => {
	...
	Object.keys(prova_array).forEach((index) => {
		row = worksheet.addRow({
			...
			totale_peso_turno_1: {
				formula: createRoundingFormula(
					Number(prova_array[index].totale_peso_1),
				),
			},
			totale_balle_turno_1: Number(prova_array[index].totale_balle_1),
			totale_peso_turno_2: {
				formula: createRoundingFormula(
					Number(prova_array[index].totale_peso_2),
				),
			},
			totale_balle_turno_2: Number(prova_array[index].totale_balle_2),
			totale_peso_turno_3: {
				formula: createRoundingFormula(
					Number(prova_array[index].totale_peso_3),
				),
			},
	...
	data.forEach((report) => {
		report.forEach((_item, rowIndex) => {
			const rowNum = rowIndex + 5;
			worksheet.getCell(`K${rowNum}`).value = {
				formula: `SUM(F${rowNum},H${rowNum},J${rowNum})`,
			};
	...
	const letter = reportInfo.includes("impianto-ab")
		? "A/B"
		: reportInfo.includes("impianto-a")
			? "A"
			: reportInfo.includes("impianto-b")
				? "B"
				: null;

	for (let i = 0; i < dimensione; ++i) {
		worksheet.getCell(`A${i + 5}`).value = letter;
		worksheet.getCell(`A${i + 5}`).alignment = {
			vertical: "middle",
			horizontal: "center",
		};
		worksheet.getCell(`B${i + 5}`).alignment = {
			vertical: "middle",
			horizontal: "center",
		};
	}
	...
	worksheet.getCell("A1").value =
		`IMPIANTO ${letter} - REPORT GIORNALIERO PER TURNI DEL ${dateStringNotSplitted}`;
	...
	var charInizio = "E";
	var indiceInizioTotali;
	var sum;
	for (let y = 0; y < 6; ++y) {
		indiceInizioTotali = 0;

		Object.keys(specialIndex).forEach((key) => {
			sum = "SUM(";
			for (let i = 0; i < specialIndex[key].length; ++i) {
				sum +=
					i === specialIndex[key].length - 1
						? `${charInizio}${specialIndex[key][i]})`
						: `${charInizio}${specialIndex[key][i]},`;
			}

			if (charInizio.charCodeAt(0) % 2 !== 0) {
				worksheet.getCell(
					`${charInizio}${dimFixed + indiceInizioTotali}`,
				).value = { formula: sum };
			} else {
				worksheet.getCell(
					`${charInizio}${dimFixed + indiceInizioTotali}`,
	...
	for (let i = 0; i < 6; ++i) {
		...
		for (let j = 0; j < indexTotalCount.size; ++j) {
			sum +=
				j === indexTotalCount.size - 1
					? `${charInizio}${Array.from(indexTotalCount)[j]})`
					: `${charInizio}${Array.from(indexTotalCount)[j]},`;
		}

		if (charInizio.charCodeAt(0) % 2 !== 0) {
			worksheet.getCell(`${charInizio}${dimFixed + 3}`).value = {
				formula: sum,
			};
		} else {
			worksheet.getCell(`${charInizio}${dimFixed + 3}`).value = {
	...
	applyBorders({
		col: "A",
		start: gapInizioConteggioCelle,
		end: dimensione + gapInizioConteggioCelle,
	});
	applyBorders({
		col: "B",
		start: gapInizioConteggioCelle,
	...
	applyBorders({
		col: "C",
		start: gapInizioConteggioCelle,
		end: dimensione + gapFineConteggioCelle,
	});
	applyBorders({
		col: "D",
		start: gapInizioConteggioCelle,
	...
	applyBorders({
		col: "E",
		start: gapInizioConteggioCelle - 1,
		end: dimensione + gapFineConteggioCelle,
	});
	applyBorders({
		col: "F",
	...
	worksheet.getCell(`F${dimensione + gapFineConteggioCelle}`).value = {
		formula: `SUM(F5:F${dimensione + gapInizioConteggioCelle})`,
	};
	worksheet.getCell(`G${dimensione + gapFineConteggioCelle}`).value = {
		formula: `SUM(G5:G${dimensione + gapInizioConteggioCelle})`,
	};
	worksheet.getCell(`H${dimensione + gapFineConteggioCelle}`).value = {
		formula: `SUM(H5:H${dimensione + gapInizioConteggioCelle})`,
	};
	worksheet.getCell(`I${dimensione + gapFineConteggioCelle}`).value = {
		formula: `SUM(I5:I${dimensione + gapInizioConteggioCelle})`,
	};
	worksheet.getCell(`J${dimensione + gapFineConteggioCelle}`).value = {
		formula: `SUM(J5:J${dimensione + gapInizioConteggioCelle})`,
	};
...
const ExportReport = ({
	...
	const hookDownload = async () => {
		if (!dateForReport) {
			...
			await handleDownload(data, reportFor, dateForReport);
		}
	};
