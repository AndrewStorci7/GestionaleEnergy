...
describe("Alert Component", () => {
	...
	it("renders error alert correctly", () => {
		...
		expect(
			screen.getByText("Errore: Something went wrong"),
		).toBeInTheDocument();
	...
	it("renders update form for presser", () => {
		const mockUpdateValuesBale = require("@/app/components/main/UpdateBale").default;
		...
		// );
		const lastCallArgs =
			mockUpdateValuesBale.mock.calls[
				mockUpdateValuesBale.mock.calls.length - 1
			][0];
...
describe("AlertProvider", () => {
	it("provides alert context to children", () => {
		const TestComponent = () => {
			...
			return (
				<button onClick={() => showAlert({ title: "Test" })}>Show Alert</button>
			);
		};
		...
		const TestComponent = () => {
			...
			return (
				<div>
					...
					<button onClick={hideAlert} data-testid="hide-alert">
						Hide
					</button>
				</div>
			);
	...
	it("throws error when used outside provider", () => {
		...
		expect(
			screen.getByText("useAlert must be used within an AlertProvider"),
		).toBeInTheDocument();
	});
});
